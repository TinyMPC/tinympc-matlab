classdef TinyMPC < handle
    ...
    methods (Access = private)
        ...
        function copy_build_artifacts(~, output_dir)
            ...
            try
                % Get the directory of this class file to locate the codegen_src directory
                class_path = mfilename('fullpath');
                [class_dir, ~, ~] = fileparts(class_path);
                codegen_src_path = fullfile(class_dir, 'codegen_src');

                fprintf('Copying all contents from codegen_src to output directory...\n');
                fprintf('codegen_src path: %s\n', codegen_src_path);
